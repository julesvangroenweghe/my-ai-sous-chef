...
import {
  ...
  ChefHat, AlertTriangle, Sparkles, GripVertical
} from 'lucide-react'
          ...
          {sortedCategories.map(cat => {
            ...
            return (
              <div key={cat}>
                ...
                {/* Dishes — 3-column-like compact layout */}
                        ...
                        <div className={`px-3 py-1.5 flex items-center gap-2 group border-b border-[#F0E8D8] ${
                          ...
                          {isEditingTitle ? (
                            ...
                            <>
                              ...
                              {/* Action buttons — always slightly visible */}
                              ...
                              <>
                                {mainComps.map((comp, i) => (
                                  <ComponentRowItem
                                    key={comp.id}
                                    comp={comp}
                                    isLast={i === mainComps.length - 1 && subGroups.length === 0}
                                ...
                                {subGroups.map(([grpName, comps], gi) => (
                                  <div key={grpName}>
                                    ...
                                    {comps.map((comp, i) => (
                                      <ComponentRowItem
                                        key={comp.id}
                                        comp={comp}
                                        isLast={i === comps.length - 1 && gi === subGroups.length - 1}
                                        editingId={editingComponent}
                                        editForm={editForm}
                                        setEditForm={setEditForm}
                                        onStartEdit={startEdit}
...
function ComponentRowItem({
  ...
  // Build inline display string like sandbox: naam (qty unit) (bereiding)
  ...
  return (
    <div className={`group px-3 py-0.5 flex items-center gap-1.5 hover:bg-[#F5EDE0]/40 transition-colors border-t border-[#F5EDE0] first:border-t-0`}>
      ...
      {/* Buttons: always 30% visible, full on hover */}
