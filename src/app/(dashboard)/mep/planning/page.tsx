...
function StatusBadge({ status, mepStatus }: { status: string; mepStatus: string | null }) {
  if (status === 'approved') {
    return (
...
function EventRow({ event }: { event: MepEvent }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-[#F2E8D5]/60 transition-all group">
      {/* Date */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <StatusBadge status={event.status} mepStatus={event.mep_status} />
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-xs">
            ...
            {totalPax > 0 && (
              <span className="text-[#9E7E60]">· {totalPax} pers.</span>
            )}
            {approvedCount > 0 && (
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: events, error } = await supabase
        .from('events')
        .select('id, name, event_date, status, mep_status, num_persons, price_per_person, location, event_type, contact_person, start_time, end_time')
      ...
      // Load dish counts + AI suggestion counts per event
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {loading ? (
        ...
        <div>
          {weeks.map((group) => (
            <WeekCard key={`${group.year}-${group.weekNum}`} group={group} />
          ))}
        </div>
      )}
    </div>
