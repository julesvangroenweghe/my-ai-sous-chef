...
const docTypeOptions: { value: DocumentType | 'auto'; label: string; sub: string }[] = [
  { value: 'auto', label: 'Automatisch detecteren', sub: 'AI bepaalt het type' },
  { value: 'pricelist', label: 'Prijslijst', sub: 'Leverancierscatalogus met prijzen' },
  { value: 'invoice', label: 'Factuur', sub: 'Aankoopfactuur leverancier' },
  { value: 'mep', label: 'MEP Lijst', sub: 'Mise en place productielijst' },
  { value: 'recipe', label: 'Recept', sub: 'Recept of bereidingswijze' },
]
          ...
          {state === 'error' && (
            <div className="mt-4 p-4 rounded-xl bg-red-50 border border-red-100 flex items-start gap-3">
              ...
              <div>
                ...
                <button onClick={resetScan} className="text-xs text-red-600 hover:text-red-700 mt-1 font-medium">
                  Opnieuw proberen
                </button>
              </div>
            </div>
                    ...
                    <tbody>
                      {(result.data.products as Array<Record<string, unknown>> || []).map((item, i) => (
                        <tr key={i} className="border-b border-stone-50 hover:bg-stone-50/50">
                          ...
                          <td className="py-2.5 px-3 text-right font-mono text-[#5C4730]">
                            {item.price_per_kg ? formatCurrency(item.price_per_kg as number) : '-'}
                          </td>
                    ...
                    <tfoot>
                      <tr className="border-t border-stone-200">
                        ...
                        <td className="py-3 px-3 text-right font-mono font-bold text-stone-900">
                          {formatCurrency(result.data.total_amount as number)}
                        </td>
                        <td />
                      </tr>
                    </tfoot>
