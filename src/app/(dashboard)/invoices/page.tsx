...
function EmptyInvoices() {
 return (
   <div className="card p-12 text-center animate-scale-in">
     ...
     <h3 className="font-display text-xl font-semibold text-stone-900 mb-2">Nog geen facturen gescand</h3>
     <p className="text-stone-500 text-sm max-w-[45ch] mx-auto mb-8 leading-relaxed">
       Upload een leveranciersfactuur en onze OCR extraheert prijzen, producten en hoeveelheden. Bijgewerkte prijzen worden automatisch doorgevoerd in al je recepten.
     </p>
 ...
 return (
   <div className="space-y-8">
     <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fade-in">
       <div>
         <h1 className="font-display text-3xl font-bold text-stone-900 tracking-tight">Facturen</h1>
         ...
         {invoices.map((inv, i) => (
           ...
           >
             ...
             <div className="flex-1 min-w-0">
               <p className="font-medium text-stone-900 text-sm truncate">{inv.supplier_name || 'Onbekende leverancier'}</p>
               <p className="text-xs text-stone-400 mt-0.5">
                 {inv.invoice_number && `#${inv.invoice_number} · `}
                 {inv.invoice_date ? new Date(inv.invoice_date).toLocaleDateString('nl-BE') : 'Geen datum'}
               </p>
             </div>
