...
const statusConfig: Record<string, { label: string; dot: string; bg: string }> = {
  ...
  generated: { label: 'Gegenereerd', dot: 'bg-sky-400', bg: 'bg-sky-50 text-sky-700' },
...
export default function EventsPage() {
  ...
  const totalRevenue = upcoming.reduce(
    (sum, e) => sum + (e.num_persons || 0) * (e.price_per_person || 0),
    0
  )
  ...
  const eventsNeedingMep = upcoming.filter((e) => {
    ...
    const needsMep = eventDate <= sevenDaysFromNow && eventDate >= now
    const hasMep = e.status === 'approved' || e.status === 'generated' || e.status === 'completed'
    return needsMep && !hasMep
  })
  ...
  return (
    <div className="space-y-8">
      ...
      {/* MEP Warning Banner */}
      {eventsNeedingMep.length > 0 && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl p-4 animate-slide-up opacity-0" style={{ animationFillMode: 'forwards' }}>
          <div className="w-8 h-8 rounded-xl bg-amber-100 flex items-center justify-center shrink-0 mt-0.5">
            <AlertTriangle className="w-4 h-4 text-amber-600" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-amber-800">
              {eventsNeedingMep.length === 1
                ? '1 event deze week zonder goedgekeurde MEP'
                : `${eventsNeedingMep.length} events deze week zonder goedgekeurde MEP`}
            </p>
            <p className="text-xs text-amber-600 mt-0.5">
              {eventsNeedingMep.map((e) => e.name).join(', ')}
            </p>
          </div>
          <Link
            href="/mep"
            className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 bg-amber-100 hover:bg-amber-200 border border-amber-200 text-amber-700 text-xs font-semibold rounded-lg transition-all"
          >
      ...
      {upcoming.length > 0 && (
        <div
          className="grid grid-cols-2 md:grid-cols-4 gap-3 animate-slide-up opacity-0"
              ...
              <div className="space-y-2">
                {upcoming.map((event, i) => {
                  ...
                  const monthShort = date
                    .toLocaleDateString('nl-BE', { month: 'short' })
                    .toUpperCase()
