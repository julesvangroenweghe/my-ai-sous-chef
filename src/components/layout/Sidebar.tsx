...
const THEMES = {
  keuken: {
    ...
    textNormal: '#5C4730',
    textSub: '#9C8060',
    textSubActive: '#C4791A',
    iconActive: '#E8A040',
    iconNormal: '#8B7050',
    scanBg: 'rgba(232,160,64,0.07)',
    scanBorder: 'rgba(232,160,64,0.35)',
    scanBorderActive: '#E8A040',
    logoColor: '#E8A040',
    kitchenColor: '#B5631A',
    kitchenSub: '#9C8060',
    deptLabel: null, // geen banner voor keuken
    toggleBg: '#E8A040',
    toggleShadow: 'rgba(232,160,64,0.4)',
  },
  sales: {
    sidebar: '#EBF4E8',
    border: '#C4DCC0',
    ...
    textNormal: '#2A4025',
    textSub: '#5A7A52',
    textSubActive: '#2D6A1E',
    iconActive: '#2D6A1E',
    iconNormal: '#5A7A52',
    scanBg: 'rgba(45,106,30,0.07)',
    scanBorder: 'rgba(45,106,30,0.35)',
    scanBorderActive: '#2D6A1E',
    logoColor: '#3A8026',
    kitchenColor: '#1A4510',
    kitchenSub: '#5A7A52',
    deptLabel: 'Sales',
    toggleBg: '#2D6A1E',
    toggleShadow: 'rgba(45,106,30,0.4)',
  },
  logistiek: {
    sidebar: '#E8EEF8',
    border: '#BCCCE8',
    ...
    textNormal: '#1A2A4A',
    textSub: '#4A6090',
    textSubActive: '#1E3F8A',
    iconActive: '#1E3F8A',
    iconNormal: '#4A6090',
    scanBg: 'rgba(30,63,138,0.07)',
    scanBorder: 'rgba(30,63,138,0.35)',
    scanBorderActive: '#1E3F8A',
    logoColor: '#2850A8',
    kitchenColor: '#122660',
    kitchenSub: '#4A6090',
    deptLabel: 'Logistiek',
    toggleBg: '#1E3F8A',
    toggleShadow: 'rgba(30,63,138,0.4)',
  },
}
...
const keukentNav = [
  { href: '/dashboard', label: 'Dashboard', icon: 'grid', sub: null },
  {
    ...
    children: [
      ...
      { href: '/draaiboek', label: 'Draaiboek', icon: 'timeline' },
    ],
  },
  {
    href: '/mep', label: 'MEP', icon: 'clipboard', sub: 'Mis en place',
    children: [
  ...
  {
    href: '/keuken', label: 'Keuken', icon: 'book', sub: 'Recepten & stijl',
    children: [
  ...
  {
    href: '/producten', label: 'Producten', icon: 'leaf', sub: 'Ingrediënten & leveranciers',
    children: [
  ...
  {
    href: '/business', label: 'Business', icon: 'chart', sub: 'Kosten & engineering',
    children: [
  ...
  {
    href: '/communicatie', label: 'Communicatie', icon: 'mail', sub: 'Mail & agenda',
    children: [
...
const salesNav = [
  { href: '/dashboard', label: 'Dashboard', icon: 'grid', sub: null },
  {
    href: '/sales', label: 'Pipeline', icon: 'pipeline', sub: 'Optie → Betaald',
    children: [
      { href: '/sales/pipeline', label: 'Kanban', icon: 'pipeline' },
      { href: '/sales/klanten', label: 'Klanten', icon: 'users' },
    ],
  },
  {
    href: '/events', label: 'Events', icon: 'calendar', sub: 'Offertes & voorstellen',
    children: [
      { href: '/events', label: 'Events', icon: 'calendar' },
    ],
  },
  {
    href: '/sales/communicatie', label: 'Communicatie', icon: 'mail', sub: 'Mail & agenda',
    children: [
      { href: '/inbox', label: 'Inbox', icon: 'mail' },
      { href: '/calendar', label: 'Kalender', icon: 'cal' },
    ],
...
const logistiekNav = [
  { href: '/dashboard', label: 'Dashboard', icon: 'grid', sub: null },
  {
    href: '/paklijsten', label: 'Paklijsten', icon: 'checklist', sub: 'Materiaal per event',
    children: [
      { href: '/paklijsten', label: 'Overzicht', icon: 'checklist' },
    ],
  },
  {
    href: '/events', label: 'Events', icon: 'calendar', sub: 'Planning',
    children: [
      { href: '/events', label: 'Events', icon: 'calendar' },
    ],
  },
...
function NavIcon({ type, active, theme }: { type: string; active: boolean; theme: typeof THEMES.keuken }) {
  const color = active ? theme.iconActive : theme.iconNormal
  const s = 16
  ...
  const icons: Record<string, JSX.Element> = {
    ...
    sprout: <svg width={s} height={s} fill="none" stroke={color} strokeWidth="1.5" viewBox="0 0 24 24"><path d="M7 20h10"/><path d="M10 20c5.5-2.5.8-6.4 3-10"/><path d="M9.5 9.4c1.1.8 1.8 2.1 2 3.5c.2-1.4.9-2.7 2-3.5-1.1-.7-2.4-.8-3.6-.7-.4 0-.8.1-1.2.2.2.2.5.3.8.5z"/></svg>,
    ...
    users: <svg width={s} height={s} fill="none" stroke={color} strokeWidth="1.5" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
    ...
    timeline: <svg width={s} height={s} fill="none" stroke={color} strokeWidth="1.5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
    pipeline: <svg width={s} height={s} fill="none" stroke={color} strokeWidth="1.5" viewBox="0 0 24 24"><rect x="2" y="7" width="4" height="10" rx="1"/><rect x="10" y="4" width="4" height="16" rx="1"/><rect x="18" y="9" width="4" height="8" rx="1"/></svg>,
  }
...
type NavItem = { href: string; label: string; icon: string; sub?: string | null; children?: NavChild[] }

function NavItemComponent({ item, pathname, theme, onNavigate }: {
  item: NavItem; pathname: string; theme: typeof THEMES.keuken; onNavigate?: () => void
}) {
  ...
  const isParentActive = item.children ? anyChildActive
  ...
  const itemStyle = (active: boolean): React.CSSProperties => ({
    ...
    borderLeft: active ? `2px solid ${theme.accentBorder}` : '2px solid transparent',
    backgroundColor: active ? theme.accentActive : 'transparent',
    cursor: 'pointer', width: 'calc(100% - 12px)', textAlign: 'left' as const,
  })

  const textStyle = (active: boolean): React.CSSProperties => ({
    fontSize: 13, fontWeight: active ? 500 : 400,
    color: active ? theme.textActive : theme.textNormal, flex: 1,
  })

  const subStyle = (active: boolean): React.CSSProperties => ({
    display: 'block', fontSize: 9,
    color: active ? theme.textSubActive : theme.textSub,
    lineHeight: 1.2, marginTop: 1, fontWeight: 400,
  })

  if (item.children) {
    return (
      <div>
        <button onClick={() => setOpen(o => !o)} style={itemStyle(isParentActive)}>
          <NavIcon type={item.icon} active={isParentActive} theme={theme} />
          <span style={textStyle(isParentActive)}>
            {item.label}
            {item.sub && <span style={subStyle(isParentActive)}>{item.sub}</span>}
          </span>
          <svg width={12} height={12} fill="none" stroke={isParentActive ? theme.accentBorder : theme.textSub}
        ...
        {open && (
          <div style={{ marginLeft: 28, marginBottom: 4, borderLeft: `1px solid ${theme.innerBorder}`, paddingLeft: 8 }}>
            {item.children.map(child => {
              ...
              return (
                <Link key={child.href} href={child.href} onClick={onNavigate} style={{
                  ...
                  padding: '6px 10px', margin: '1px 4px', borderRadius: 5, textDecoration: 'none',
                  backgroundColor: childActive ? theme.accentActive : 'transparent',
                  borderLeft: childActive ? `2px solid ${theme.accentBorder}` : '2px solid transparent',
                }}>
                  <NavIcon type={child.icon} active={childActive} theme={theme} />
                  <span style={{ fontSize: 12, fontWeight: childActive ? 500 : 400, color: childActive ? theme.textActive : theme.textSub }}>
                    {child.label}
                  </span>
                </Link>
              )
  ...
  return (
    <Link href={item.href} onClick={onNavigate} style={itemStyle(active) as React.CSSProperties}>
      <NavIcon type={item.icon} active={active} theme={theme} />
      <span style={textStyle(active)}>
        {item.label}
        {item.sub && <span style={subStyle(active)}>{item.sub}</span>}
      </span>
    </Link>
  )
}
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const navItems = dept === 'sales' ? salesNav : dept === 'logistiek' ? logistiekNav : keukentNav
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        ...
        if (res.ok) { const data = await res.json(); setUnreadAlerts(data.alerts?.length || 0) }
      } catch { /* silent */ }
    }
    fetchAlerts()
  ...
  const handleNavigate = () => { if (onClose) onClose() }

  // Department labels + icons
  const deptConfig: Record<Department, { label: string; icon: JSX.Element }> = {
    keuken: {
      label: 'Keuken',
      icon: <svg width={13} height={13} fill="none" stroke={theme.logoColor} strokeWidth="1.5" viewBox="0 0 24 24"><path d="M3 11l19-9-9 19-2-8-8-2z"/></svg>,
    },
    sales: {
      label: 'Sales',
      icon: <svg width={13} height={13} fill="none" stroke={theme.logoColor} strokeWidth="1.5" viewBox="0 0 24 24"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>,
    },
    logistiek: {
      label: 'Logistiek',
      icon: <svg width={13} height={13} fill="none" stroke={theme.logoColor} strokeWidth="1.5" viewBox="0 0 24 24"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>,
    },
  }

  return (
    <aside style={{
      position: 'fixed', left: 0, top: 0, height: '100vh', width: 232,
      ...
      <div style={{ padding: '20px 20px 16px', borderBottom: `1px solid ${theme.innerBorder}`, flexShrink: 0 }}>
        ...
        {/* Kitchen info + department badge */}
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${theme.innerBorder}`, display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{ width: 5, height: 5, borderRadius: '50%', backgroundColor: theme.accent, flexShrink: 0 }} />
          <span style={{ fontSize: 11, fontFamily: 'Georgia, serif', color: theme.kitchenColor, letterSpacing: '0.06em', textTransform: 'uppercase', fontWeight: 500 }}>
            {kitchen?.name || 'Mijn Keuken'}
          </span>
          <span style={{ fontSize: 10, color: theme.kitchenSub, marginLeft: 2 }}>&middot; {kitchen?.kitchen_type || 'Catering'}</span>
        </div>

        {/* Department badge (altijd zichtbaar) */}
        <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', borderRadius: 4, backgroundColor: `${theme.accent}18`, border: `1px solid ${theme.accent}30` }}>
      ...
      <div style={{ padding: '8px 12px 4px', flexShrink: 0 }}>
        <Link href="/scan" onClick={handleNavigate} style={{
          display: 'flex', alignItems: 'center', gap: 9, padding: '9px 12px', borderRadius: 7, textDecoration: 'none',
          backgroundColor: scanActive ? theme.accentActive : theme.scanBg,
          border: `1px solid ${scanActive ? theme.scanBorderActive : theme.scanBorder}`,
        }}>
          <NavIcon type="scan" active={true} theme={theme} />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: theme.textActive, lineHeight: 1.2 }}>Scan & OCR</div>
            <div style={{ fontSize: 10, color: theme.textSub, lineHeight: 1.2 }}>Prijslijst, factuur, recept</div>
          </div>
          <svg width={10} height={10} fill="none" stroke={theme.accent} strokeWidth="2" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
        </Link>
      </div>

      <div style={{ borderTop: `1px solid ${theme.innerBorder}`, margin: '6px 14px 0', flexShrink: 0 }} />

      {/* Bottom nav */}
      <div style={{ padding: '6px 0 4px', flexShrink: 0 }}>
        <Link href="/alerts" onClick={handleNavigate} style={{
          display: 'flex', alignItems: 'center', gap: 10, padding: '7px 18px', margin: '1px 6px', borderRadius: 6, textDecoration: 'none',
          backgroundColor: alertsActive ? theme.accentActive : 'transparent',
          borderLeft: alertsActive ? `2px solid ${theme.accentBorder}` : '2px solid transparent',
        }}>
          <div style={{ position: 'relative' }}>
            <NavIcon type="bell" active={alertsActive || unreadAlerts > 0} theme={theme} />
            {unreadAlerts > 0 && (
          ...
          <span style={{ fontSize: 13, fontWeight: (alertsActive || unreadAlerts > 0) ? 500 : 400, color: (alertsActive || unreadAlerts > 0) ? theme.textActive : theme.textNormal }}>
            Meldingen
            {unreadAlerts > 0 && <span style={{ marginLeft: 6, fontSize: 10, color: theme.textSub, fontWeight: 400 }}>{unreadAlerts} nieuw</span>}
          </span>
        </Link>
        ...
        {bottomItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={handleNavigate} style={{
              ...
              <NavIcon type={item.icon} active={active} theme={theme} />
              <span style={{ fontSize: 13, fontWeight: active ? 500 : 400, color: active ? theme.textActive : theme.textNormal }}>{item.label}</span>
            </Link>
          )
        })}
        ...
        <button onClick={signOut} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 18px', margin: '1px 6px', borderRadius: 6, background: 'none', border: 'none', borderLeft: '2px solid transparent', cursor: 'pointer', width: 'calc(100% - 12px)' }}>
          <svg width="16" height="16" fill="none" stroke={theme.textSub} strokeWidth="1.5" viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
          <span style={{ fontSize: 13, color: theme.textSub }}>Afmelden</span>
        </button>
      </div>
      ...
      <button
        ...
        style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '4px 12px 0', padding: '7px 10px', borderRadius: 6, background: 'rgba(255,255,255,0.6)', border: `1px solid ${theme.innerBorder}`, cursor: 'pointer', width: 'calc(100% - 24px)', flexShrink: 0 }}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={theme.textSub} strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <span style={{ fontSize: 12, color: theme.textSub, flex: 1, textAlign: 'left' }}>Zoeken...</span>
        <kbd style={{ fontSize: 10, color: theme.textSub, background: 'rgba(0,0,0,0.04)', border: `1px solid ${theme.innerBorder}`, borderRadius: 3, padding: '1px 4px' }}>&crarr;K</kbd>
      </button>

      {/* Food cost widget — alleen keuken */}
      {dept === 'keuken' && (
        <div style={{ margin: '8px 12px 16px', padding: '10px 12px', borderRadius: 6, backgroundColor: 'rgba(255,255,255,0.5)', border: `1px solid ${theme.innerBorder}`, flexShrink: 0 }}>
          <div style={{ fontSize: 9, color: theme.textSub, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 6 }}>Food Cost Target</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ flex: 1, height: 3, backgroundColor: theme.innerBorder, borderRadius: 2 }}>
              <div style={{ width: '75%', height: '100%', backgroundColor: theme.accent, borderRadius: 2 }} />
            </div>
            <span style={{ fontSize: 11, color: theme.textNormal, whiteSpace: 'nowrap' }}>25&ndash;30%</span>
          </div>
        </div>
      )}
