...
// Two modes:
//   1. Event has explicit allergens (e.g. "noten, kiwi") → mark those as fromEvent=true
//      AND also show all other allergens found in components (fromEvent=false)
//   2. No event allergens → show all allergens found in menu (full transparency)
...
function buildAllergenMap(eventAllergens: string | null | undefined, dishes: MepListDish[]): AllergenDishMap[] {
  ...
  if (eventAllergenList.length > 0) {
    ...
    // Guest allergens first (fromEvent=true)
    ...
    // Other allergens in menu (not declared by guest, but present)
    for (const [foundA, dishSet] of allFound) {
    ...
    // Mode 2: No guest allergens → show all found allergens for full transparency
        ...
        <View key={gi} style={group.groupName ? S.componentGroup : undefined}>
          ...
          {group.items.map((comp, ci) => {
            ...
            const compAllergens = comp.allergens
              ...
              : []
            // Highlight component name in red+bold if it matches a guest allergen
...
export function MepListDocument({ data }: { data: MepListData }) {
  ...
  // Set of guest allergens for component highlighting in dishes
